        ...
        <div class="awesome-cards-heading">
            <div class="row">
                <div class="col-lg-9 col-md-10 text-center mt-5 offset-lg-3 offset-md-1">
                    <h1 class="text-muted mt-5">Business as usual</h1>
                    <?php if($this->session->flashdata('order_processed')): ?>
                            <?php echo '<p class="alert alert-info">'.$this->session->flashdata('order_processed'). ' <i class="fa fa-check"></i></p>'; ?>
                    <?php endif; ?>
                </div>
            </div>
        </div>
                            ...
                            <tbody id="showdata">
                                <?php foreach($orders as $order):?>
                                    <tr>
                                        ...
                                        <td><?php echo $order['name_of_card'];?></td>
                                        <td><?php echo $order['user_name'];?></td>
                                        <td><?php echo $order['receiver'];?></td>
                                        <td><?php echo $order['receiver_location'];?></td>
                                        <td><?php echo $order['senders_cellphone'];?></td>
                                        <td><?php echo $order['include_number'];?></td>
                                        <td><?php echo $order['receiver_cellphone'];?></td>
                                        <td><?php echo $order['message_to_receiver'];?></td>
                                        <td>
                                            <?php echo form_open('awesome_admin_process_order')?>
